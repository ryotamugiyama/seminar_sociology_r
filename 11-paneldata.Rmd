...
先に説明したとおり、1回しか観察を得られていない個人は推定に貢献しないため除外される。`fixest::feols()`では、次のようなメッセージが表示される：
...
固定効果モデルを推定する場合、同一個人の観察は互いに独立ではないため、通常クラスター・ロバスト標準誤差を用いる。Rでは、`fixest::feols()`関数の`vcov`引数で`"cluster"`を指定することによって、クラスター・ロバスト標準誤差を計算することができる。クラスター化の単位は、`panel.id`引数で指定した個人を表す変数となる。
...
model_fe <- feols(data = nls_sample, 
            ln_wage ~ union + age + I(age^2) + msp + race + collgrad | id,
            panel.id = "id", vcov = "cluster")
...
model_pols1 <- feols(data = nls_sample2, 
              ln_wage ~ union + age + I(age^2) + msp, 
              panel.id = "id", vcov = "cluster")

# Pooled OLS - 時間不変の変数あり
model_pols2 <- feols(data = nls_sample2, 
              ln_wage ~ union + age + I(age^2) + msp + race + collgrad, 
              panel.id = "id", vcov = "cluster")
...
modelsummary(list(
  "Pooled OLS (1)" = model_pols1, 
  "Pooled OLS (2)" = model_pols2, 
  "Fixed effect" = model_fe),
  stars = TRUE,gof_map = 
    c("nobs", "r.squared", "FE: id"))
```

### 個人間効果と個人内効果

Pooled OLSで得られた`union`の係数は、以下の2つの異なる差を反映している[^11-paneldata-1]。
