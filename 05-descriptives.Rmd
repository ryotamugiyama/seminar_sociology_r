...
カテゴリ変数の場合は平均値や標準偏差のような要約統計量を計算することはできない（意味がない）。最も基本的な集計は、各カテゴリにそれぞれ何人いるのかを確認することだ。`with(table())`を使うことで各カテゴリの人数（度数）をみることができる。
...
度数だけでなく、各職業にどれくらいの割合の人が含まれているのかも重要だ。先ほどのコマンドに`prop.table()`を加えることで、各カテゴリに含まれる人が全体に占める割合を計算できる。
...
このように度数を数える場合には、`count()`を使うのもよい。次のように集計したい変数を指定することで、各カテゴリの度数を数えることができる。
...
先ほどと同様に、度数を数えるだけではなく、割合を計算するとより見やすいだろう。

```{r}
piaac %>% 
  count(occupation) %>% 
  mutate(sum = sum(n)) %>% # すべての度数を合計した値を入れた列を作成する
  mutate(prop = n / sum) # 各職業の度数を合計で割った値を求める
```

### 分布の可視化：棒グラフ

こうして集計した値を棒グラフにしすると見やすいかもしれない。このようなときにはどうすればよいだろう？先ほど紹介した`with(table())`と`count()`、それぞれを使った場合の棒グラフを見てみよう。

#### with(table())を使う場合

`with(table())`を使う場合には、集計表を`as.data.frame()`コマンドを使ってデータフレーム形式に変換する必要がある。
...
データフレーム形式に変換すると、このように、1行が1つの職業を表す形式へと変換できる。Freqという列には、先ほど計算した割合が入っている。これを`ggplot()`に渡してやることで、棒グラフをつくることができる。

棒グラフを作るときのコマンドは、`geom_col()`である。
...
piaac %>% 
  ...
  prop.table() %>% 
  as.data.frame() %>% 
  ggplot(aes(x = occupation, y = Freq)) + # x軸をoccupation, y軸をFreqとして指定
  geom_col()
```
...
`count()`を使う場合には先ほどのようなデータフレーム形式への変換は必要なく、結果をそのまま`ggplot()`に渡してやるだけでよい。
...
piaac %>% 
  ...
  mutate(sum = sum(n)) %>% 
  mutate(prop = n / sum) %>% 
  ggplot(aes(x = occupation, y = prop)) + # x軸をoccupation, y軸をpropとして指定
  geom_col() + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + # X軸を90度傾ける
  labs(x = "職業", y = "割合") # x軸とy軸に名前をつける
```

図を作る場合には、どちらかというとこちらのほうがシンプルなやりかただろう。特に [第6章](#tabulate)でクロス集計表の棒グラフを作る場合には、こちらのやりかたのほうがわかりやすくなる。
...
何らかの属性などでサンプルを分けて比較分析する場合には、属性ごとの記述統計量を示すとよい。これも、`tbl_summary()`のなかでオプションを指定することで簡単に実行することができる。たとえば性別（gender）で分ける場合には、次のようにする。
...
自分はたいていwidthを5、heightを4に設定している。ただし、より複雑な図であれば、適宜サイズの変更が必要だ。
...
どうやら`gtsummary::tbl_summary()`はカテゴリ変数の値の種類が少ないと、連続変数ではなくカテゴリ変数のように集計するという親切（？）な機能があるようだ。オプションとして`type = list()`で連続変数であることを記載することで、この親切を抑制することができる。

```{r}
piaac_selected2 %>% 
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"),
              type = list(health ~ "continuous")) # すべての連続変数を強制的に連続変数として表記)
```