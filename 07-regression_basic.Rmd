...
### Excelファイルに書き出す {#regression_output_excel}

Excelファイルに書き出したい場合は、末尾の「.docx」の部分を「.xlsx」に変えればよい。ただし、Excelファイルの場合は少し余分な列などが入ってしまうので、適宜手直しが必要となる。
...
modelsummary(list(reg_res),
         ...
         coef_map = c("(Intercept)" = "切片",
                         "age" = "年齢"), # 各変数に名前をつける
         gof_map = c("nobs", "r.squared"), # サンプルサイズと決定係数のみ記載する
         output = "results/regression.xlsx") # 出力先のファイル名をつける（xlsx）
```

### PowerPointファイルに書き出す {#regression_output_pptx}

結果をPowerPointに書き出すこともできるようだ。この場合は、末尾の拡張子を「.pptx」に変えるとよい。
...
modelsummary(list(reg_res),
         ...
         coef_map = c("(Intercept)" = "切片",
                         "age" = "年齢"), # 各変数に名前をつける
         gof_map = c("nobs", "r.squared"), # サンプルサイズと決定係数のみ記載する
         output = "results/regression.pptx") # 出力先のファイル名をつける（pptx）
```

### rtfファイルに書き出す {#regression_output_rtf}

rtf（リッチテキスト）形式に書き出す。このファイルをWordで開くと、比較的きれいな形式の見た目になっていることが多い。末尾の拡張子を「.rtf」に変えるとできる。
...
modelsummary(list(reg_res),
         ...
         coef_map = c("(Intercept)" = "切片",
                         "age" = "年齢"), # 各変数に名前をつける
         gof_map = c("nobs", "r.squared"), # サンプルサイズと決定係数のみ記載する
         output = "results/regression.rtf") # 出力先のファイル名をつける（rtf）
```
