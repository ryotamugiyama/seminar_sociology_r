...
パネルデータの場合、個々の観察は互いに独立ではない。ある時点で調査された個人は次も調査対象となるからである。この場合、残差が独立に同一の分布に従うIID; Independent and identically distributed）の仮定が満たされない。を意味し、観察が互いに独立であり、かつ、同じ分布に従、通常クラスター・ロバスト標準誤差を用いる。Rでは、`fixest::feols()`関数の`vcov`引数で`"cluster"`を指定することによって、クラスター・ロバスト標準誤差を計算することができる。クラスター化の単位は、`panel.id`引数で指定した個人を表す変数となる。
