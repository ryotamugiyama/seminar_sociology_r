...
カテゴリ変数の場合は平均値や標準偏差のような要約統計量を計算することはできない（意味がない）。最も基本的な集計は、各カテゴリにそれぞれ何人いるのかを確認することである。`with(table())`を使うことで各カテゴリの人数（度数）をみることができる。
...
度数だけでなく、各職業にどれくらいの割合の人が含まれているのかも重要である。先ほどのコマンドに`prop.table()`を加えることで、各カテゴリに含まれる人が全体に占める割合を計算できる。
...
自分はたいていwidthを5、heightを4に設定している。ただし、より複雑な図であれば、適宜サイズの変更が必要である。
...
どうやら`gtsummary::tbl_summary()`はカテゴリ変数の値の種類が少ないと、連続変数ではなくカテゴリ変数のように集計するという親切（？）な機能があるようである。オプションとして`type = list()`で連続変数であることを記載することで、この親切を抑制することができる。
