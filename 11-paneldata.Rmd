...
本章では、パネルデータ分析の基礎、およびパネルデータ分析で最も中心的な分析手法である固定効果モデルについて説明する。
...
### 観察数と人数 N of observations/individuals

パネルデータを分析する場合、観察数（N of observations）と人数（N of individuals）をそれぞれ示すと有益である。両者がわかることで、だいたい一人につき平均何回程度の観察数が得られているかがわかるからである。観察数は単純に行数をカウントし、人数の場合には人（id）単位で集計してからカウントすることで求めることができる。

```{r}
# 観察数（N of observations）をカウント
nls_sample %>% 
  nrow() # 行数をカウント

# 人数（N of individuals）をカウント
nls_sample %>% 
  count(id) %>% # idごとにデータを集計して、
  nrow() # 行数をカウント
```

```{r, include = FALSE}
n_obs <- nls_sample %>% nrow() # 観察数を変数に保存
n_individuals <- nls_sample %>% count(id) %>% nrow() # 人数を変数に保存
```

観察数が`r n_obs`、人数が`r n_individuals`である。したがって、だいたい一人あたり平均`r round(n_obs/n_individuals, digit = 2)`回くらいの観察が得られているということである。
...
パネルデータ分析では、ある変数の1時点前の値や1時点後の値を変数として使用することがある。このような場合には、[`dplyr::lag()`および`dplyr::lead()`関数](https://dplyr.tidyverse.org/reference/lead-lag.html)を使う。

-   `dplyr::lag()`関数は任意の行数上の値をコピーする（ラグ）。たとえばxという変数の1行上の値をコピーするには`dplyr::lag(x, n = 1)`とする。
-   `dplyr::lead()`関数は任意の行数下の値をコピーする（リード）。たとえばxという変数の1行下の値をコピーするには`dplyr::lead(x, n = 1)`とする。

パネルデータの場合、あらかじめ同一個人内で時点の昇順にデータを並べ替えておいた上で`dplyr::lag()`関数や`dplyr::lead()`関数を使う必要がある。そうでなければ、意図した値とは異なる値をコピーしてきてしまうからである。たとえば、対数賃金`ln_wage`の1時点前の値、1時点後の値をコピーした新たな変数を作りたいとする。この場合には次のようにする。
...
nls_sample <- nls_sample %>% 
  ...
  mutate(
    ln_wage_lag = dplyr::lag(ln_wage, n = 1), # 1時点前のln_wageをコピー
    ln_wage_lead = dplyr::lead(ln_wage, n = 1) # 1時点後のln_wageをコピー
  ) %>% 
  ungroup() # グループ化を解除
...
固定効果モデルの強みは、独立変数の個人内変動と従属変数との関連を明らかにすることができるという点にある。たとえば、「非正規雇用で働いていると、正規雇用で働いている場合とくらべて幸福度が悪化するだろうか？」という問いに答えたいとする[^11-paneldata-2]。もちろん、正規雇用で働いている人もいれば非正規雇用で働いている人もいる。正規雇用で働くか非正規雇用で働くかには、性別や学卒時の経済状況などといった比較的観察しやすい属性の他にも、出身家庭の豊かさ、出身大学の選抜度、パーソナリティなどといった観察しにくい属性も影響すると考えられる。固定効果モデルを用いると、仮に調査で聴取されていなかったとしても、こうした時間によって変わらない特徴をすべて統制することができる。これによって、因果効果に近づくことができる（だろう）というのが、固定効果モデルのメリットである。
...
## 補足的論点 {#paneldata_additional}

### Pooled OLSと固定効果モデルの比較 {#paneldata_pols_fe_compare}
