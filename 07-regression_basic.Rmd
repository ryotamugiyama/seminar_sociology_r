...
そのうえで、以下のように、ふつうの回帰分析と同じように実行すればよい。違うのは、関数が`lm()`ではなくて`lm_robust()`となっている点だけである。
...
となり、xが1単位増えたときに$y$は$\exp(\beta_1)$倍になる、ということがわかる。いくつか実際の値を計算してみると、次のようになる：
